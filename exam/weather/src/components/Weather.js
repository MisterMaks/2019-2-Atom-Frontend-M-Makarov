...
export class Weather extends Component {
	...
	getWeatherFromGeolocation() {
		...
		fetch(`http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&units=metric&APPID=22974c3e3aacf49aa6ed8191ccca8d39`, {
			headers: {origin: document.origin},
			method: 'GET',
		})
			.then((resp) => resp.json())
			.then((data) => {
				this.setState({location: data.name, locationID: data.id, locationTemperature: data.main.temp, locationIconURL: `http://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`});
			});
	}

	getWeatherFromCityName(event, cityName) {
		fetch(`http://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=metric&APPID=22974c3e3aacf49aa6ed8191ccca8d39`, {
			headers: {origin: document.origin},
			method: 'GET',
		})
			.then((resp) => resp.json())
			.then((data) => {
				const newCity = {id: data.id, name: data.name, temperature: data.main.temp, iconURL: `http://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`};
	...
	getWeatherFromID(cityID) {
		fetch(`http://api.openweathermap.org/data/2.5/weather?id=${cityID}&units=metric&APPID=22974c3e3aacf49aa6ed8191ccca8d39`, {
			headers: {origin: document.origin},
			method: 'GET',
		})
			.then((resp) => resp.json())
	...
	goToWeatherCityPage(event, city, cityID, currentTemperature, currentIconURL, geo) {
		fetch(`http://api.openweathermap.org/data/2.5/forecast?id=${cityID}&units=metric&APPID=22974c3e3aacf49aa6ed8191ccca8d39`, {
      headers: {origin: document.origin},
      method: 'GET',
    })
      .then((resp) => resp.json())
      .then((data) => {
				...
				const tomorrow = {temperature: forecast[8].main.temp, weatherIconURL: `http://openweathermap.org/img/wn/${forecast[8].weather[0].icon}@2x.png`};
        const dayAfterTomorrow = {temperature: forecast[16].main.temp, weatherIconURL: `http://openweathermap.org/img/wn/${forecast[16].weather[0].icon}@2x.png`};
				const thirdDay = {temperature: forecast[24].main.temp, weatherIconURL: `http://openweathermap.org/img/wn/${forecast[24].weather[0].icon}@2x.png`};
					...
					<Switch>
						...
						<Route path={"/"+this.state.selectedCityData.link}>
							<WeatherCityPage 
								...
								currentTemperature={this.state.selectedCityData.currentTemperature}
